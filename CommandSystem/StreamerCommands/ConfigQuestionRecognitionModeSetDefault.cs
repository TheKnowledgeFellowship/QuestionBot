...
{
    public class ConfigQuestionRecognitionModeSetDefault : IStreamerCommand
    {
        public string Name => "config-questionrecognitionmode-set-default";
        public string Call => @"^config\s*questionrecognitionmode\s*set\s*default\s*";
        ...
        public async Task ActionAsync(StreamerCommandArguments commandArguments)
        {
            using (var db = new CuriosityContext())
            {
                ...
                else
                {
                    ...
                    await commandArguments.Client.SendMessageAsync("The QuestionRecognitionMode has been successfully changed to the default: `both`.");
                }
            }
        }
    }
